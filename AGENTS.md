...
- **fpga-lisp** (this repo) — hardware implementation of the same language
  ...
  M28 (`447ee0e`) proved the `letrec` mechanism itself (placeholder-pair +
  `SETCDR` backpatch) with a simplified non-tail-recursive `length`. M29
  (WIP, `cec7889`) is bootstrapping the canonical tail-recursive,
  mutually-recursive `length`/`length-onto` pair `core.my` actually uses —
  needed before `reverse`/`append`/`map` can bootstrap. CI
