...
# Апаратне налаштування Tang Primer 25K (Ukrainian)

## Інтерфейс UART (COM-порти) — дійсний з 2026-08-16
Цей проєкт наразі розробляється на машині, де встановлено Windows як хост, 
а інструменти GNU/Linux (як-от компілятор CML, симулятор) виконуються через 
WSL2. 

**Увага щодо WSL2:**
Тимчасове рішення (workaround) `usbipd-win`, яке раніше використовувалося 
для прокидання USB-пристроїв до WSL2, було **відкликано** 2026-08-16 як хибний 
шлях. Воно є нестабільним і заважає роботі. Будь ласка, **НЕ запускайте** 
`usbipd attach`. Комунікація через UART та програмування (через `upload.py` 
та JTAG/`programmer_cli.exe`) повинні виконуватися безпосередньо з нативної Windows. 
Використання скриптів на базі pyserial/UART через WSL2 призводить до помилки 
`termios.error: (5, 'Input/output error')`. Це обмеження платформи WSL2, а не 
баг наших інструментів.

Встановлення Python у нативній Windows:
```
winget install -e --id Python.Python.3.12
python -m pip install pyserial
```

**Який саме COM-порт є UART:** Чіп FT2232 на платі надає два канали, які 
визначаються як `USB Serial Converter A`/`B` -> `COM3`/`COM4`. Канал B 
(`COM4` у нашому випадку) — це той, з яким успішно взаємодіють 
`upload.py`/`monitor.py`. Канал A — це JTAG-інтерфейс.

## Програмування JTAG — ВИРІШЕНО 2026-08-24

Після відключення дочірнього віртуального COM-порту (VCP) для Каналу A, інструмент 
Gowin зміг побачити пристрій на `location 449`. Програмування в SRAM успішно завершилося 
(`project.fs`, Status 0x70026020).

### Постійне програмування зовнішньої flash-пам'яті — перевірено 2026-08-24

Синтезований RTL (`092aa3b`) був успішно записаний у зовнішню SPI NOR пам'ять 
плати (операція 54, Arora V). Після повного відключення живлення та повторного 
завантаження плата зберегла образ. Підтвердження: нативне Windows-завантаження 
`bootstrap_add_demo.bin` через UART (COM4) успішно виконалося і повернуло 
`R9 = FIXNUM(7) [0x00000007]`. Це доводить можливість завантаження з flash-пам'яті, 
але не є доказом апаратної відповідності всьому стандарту мови.

### Міст команд CML (CML command bridge)

Скрипт `job_transport.py` — це неінтерактивний транспорт для нативної Windows, 
який використовується компілятором CML (`CommandFpgaTransport`). Він читає 
завдання з `stdin`, керує таймінгами скидання/HALT на `COM4` і повертає фіксований 
бінарний результат у `stdout`. Живий тест CML успішно передав на фізичну FPGA-плату 
завдання, отримав `0x00000007` і опублікував `GraphValue::LispWord(7)`. 

## Попередній блокер — частково закритий

Проблема з інтерфейсами JTAG та UART була вирішена завдяки живим тестам. Програма 
`bootstrap_add_demo` підтверджена на апаратному рівні. Проте це ще не верифікує 
всі інші програми (M17-M32); для кожного нового заявленого сценарію поведінки 
потрібен свій завантажений бінарник і спостереження за регістрами/купою/помилками.
