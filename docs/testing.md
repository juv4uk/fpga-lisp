...
iverilog -g2012 -I fpga/rtl -o tb.vvp \
  ...
  fpga/rtl/registers.sv fpga/rtl/instruction_decoder.sv fpga/rtl/upc8_unit.sv \
  fpga/rtl/sandhi_engine.sv fpga/rtl/control.sv \
...
| M30 | [`tb_bootstrap_reverse.sv`](../fpga/sim/tb_bootstrap_reverse.sv) | The `reverse-onto`/`reverse` letrec pair from `core.my`, same mechanism as M29 applied to a second mutually-recursive pair. First bootstrap demo whose result is a CONS structure rather than a fixnum, so the testbench walks `car_ram`/`cdr_ram` directly. `(reverse '(a b c)) => (c b a)`, PASSED on the first real run. |
| M31 | [`tb_bootstrap_append.sv`](../fpga/sim/tb_bootstrap_append.sv) | `append` from `core.my`: `(lambda (left right) (reverse-onto (reverse left) right))`, built on top of M30's letrec pair. Not self-referential, so no third letrec placeholder — just an ordinary closure that looks up `reverse`/`reverse-onto` by name through its captured env. `(append '(a b) '(c d)) => (a b c d)`, confirmed by a real `iverilog` run, cons-chain walked directly. Also cross-checked against my-lisp's TCP `--protocol=sexpr` oracle: `(append '(a b) '(c d)) => (a b c d)` matches. |
| M32 | [`tb_bootstrap_equal.sv`](../fpga/sim/tb_bootstrap_equal.sv) | `equal?` from `core.my`: self-referential structural equality, single letrec placeholder (like M28, unlike M30/M31's mutual pairs), first bootstrap demo with a three-clause top-level `cond`. `(equal? '(p . q) '(p . q)) => t` using two distinct cons cells with the same content, proving structural rather than pointer comparison. Edge-case checklist gathered from my-lisp's TCP oracle before writing the code (see [`equal-oracle-checklist.my`](equal-oracle-checklist.my)); confirmed cycles are out of scope (language has no `set-cdr!`/mutation). |
| M33 | [`tb_bootloader_register_init.sv`](../fpga/sim/tb_bootloader_register_init.sv) | ISA 1.1 extended boot header initializes tagged R0=3 and R1=4 before CPU start; a two-instruction program computes R2=7. Legacy headers remain unchanged. Targeted legacy gates (`tb_machine`, `tb_jf_truthiness`, `tb_monitor`) pass locally; the full regression remains a CI gate. |
| M34 | [`test_job_transport.py`](../tests/test_job_transport.py) | The host bridge accepts CML's ISA 1.1 extended frame, validates unique R0..R15 inputs and exact length, and forwards tagged words opaquely. Four targeted protocol tests pass; physical execution is not claimed. |
...
```bash
for tb in tb_cons tb_heap tb_car_cdr tb_atom_eq tb_machine tb_monitor tb_control tb_jf_truthiness tb_list tb_call tb_env tb_lambda tb_eval_atom tb_eval_quote tb_eval_cond tb_eval_apply tb_eval_primitive tb_error_recovery tb_eval_all_primitives tb_bootstrap_nullp tb_bootstrap_second tb_bootstrap_not tb_bootstrap_pair tb_bootstrap_caar tb_bootstrap_triple tb_bootstrap_third tb_setcdr tb_bootstrap_add tb_bootstrap_length tb_bootstrap_length_onto tb_bootstrap_reverse tb_bootstrap_append tb_bootstrap_equal tb_bootloader_register_init; do
