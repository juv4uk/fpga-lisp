...
| M28 | [`tb_bootstrap_length.sv`](../fpga/sim/tb_bootstrap_length.sv) | 🔴 **ACTUALLY FAILING as of 2026-08-11** — see `docs/lisp-machine-plan.md`'s M28 entry. Intends to prove the `letrec`-style mechanism (placeholder-pair-then-`SETCDR`-backpatch), but the first real `iverilog` run (not just hand-traced) halts with `R9 = TAG:SYMBOL VAL:920` (leaked param symbol `'lst`) instead of `TAG:FIXNUM VAL:3`, and the software call stack (`R11`) is non-`NIL` at halt — an unbalanced push/pop somewhere only reachable at real recursion depth. Root cause not yet found. Do not trust this row's old "PASSED" framing until it's re-verified. |
