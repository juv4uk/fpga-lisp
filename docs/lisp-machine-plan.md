...
- ✅ **M28 — `letrec`-механізм самореференційної рекурсії закритий (спрощений приклад, НЕ повна відповідність `core.my`)**: розв'язує архітектурну межу, знайдену після M25 (нижче — оригінальна нотатка, для історії) — механізм. Placeholder-пара `(name . NIL)` розширює нову env-рамку (`new_env`), closure будується з `new_env` як captured env, тоді `SETCDR` (M26) дозаписує placeholder на реальну closure — свідомо створюючи цикл `ph_pair -> closure -> new_env -> ph_pair`, завдяки якому тіло closure знаходить **саме себе** через звичайний `lookup`, без жодних змін у самому `eval`/`lookup`. [tb_bootstrap_length.sv](../fpga/sim/tb_bootstrap_length.sv), [bootstrap_length_demo.asm](../bootstrap_length_demo.asm).
  ...
  - Truthy-семантика підтверджена коректною (my-lisp, G8-аксіома): лише `NIL` — falsy, все інше truthy (включно з fixnum `0`) — точно відповідає підходу, який `isa-contract.my` вже документує для `JF`.
  ...
  - `list`/`reverse`/`append`/`map`/`filter`/`reduce` (variadic-параметричні, а не лише самореференційні) і повна взаємно-рекурсивна хвостова форма `length`/`length-onto` лишаються за межами M28 — наступний природний крок.
