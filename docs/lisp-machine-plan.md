...
- 🔴 **M28 — ФАКТИЧНО ЗЛАМАНИЙ при реальному прогоні (виявлено 2026-08-11, після встановлення iverilog локально)**: раніше цей запис стверджував ✅ на основі ручного трасування регістрів, без реального запуску в симуляторі. Перший реальний прогін (`iverilog`/`vvp`, локально встановлені після того, як диск нарешті звільнили) показав: машина **halts нормально** (це не зависання — `$finish` спрацьовує на 113 021 940 симульованих одиниць, задовго до watchdog 150 000 000), але `R9` (результат) = `TAG:SYMBOL VAL:920` (символ `'lst`, внутрішній param-symbol), а НЕ очікуваний `TAG:FIXNUM VAL:3`. Додатковий дамп усіх 16 регістрів на halt показав `R11` (програмний call-стек) = `TAG:CONS VAL:39`, а не `NIL` — стек лишився незбалансованим (є непопнуті push). Кожен окремий блок `eval_core.inc` (`cond_loop`, `try_apply`, `do_closure_apply`'s bare-symbol шлях, `chk_add`), перевірений статично, збалансований сам по собі — отже баг, найімовірніше, проявляється лише на глибині справжньої 3-рівневої самореференційної рекурсії, яку жоден попередній milestone (M11-M27) фактично не вправляв (усі раніше пройдені тести мали неглибоку чи нерекурсивну структуру виклику). Корінь ще не локалізовано — потрібне подальше трасування (напр. `$monitor` на `u_regs.regs[11]` по кожному циклу, щоб знайти точний момент розбалансування). **Не вважайте M28 доведеним, доки цей запис не оновиться на PASSED з підтвердженим прогоном.**
  - Оригінальний (тепер спростований) запис нижче лишається для історії ходу міркувань, але його висновок "✅ закритий" наразі невірний: розв'язує архітектурну межу, знайдену після M25 (нижче — оригінальна нотатка, для історії) — механізм. Placeholder-пара `(name . NIL)` розширює нову env-рамку (`new_env`), closure будується з `new_env` як captured env, тоді `SETCDR` (M26) дозаписує placeholder на реальну closure — свідомо створюючи цикл `ph_pair -> closure -> new_env -> ph_pair`, завдяки якому тіло closure знаходить **саме себе** через звичайний `lookup`, без жодних змін у самому `eval`/`lookup`. [tb_bootstrap_length.sv](../fpga/sim/tb_bootstrap_length.sv), [bootstrap_length_demo.asm](../bootstrap_length_demo.asm).
