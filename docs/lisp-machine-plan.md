...
- ✅ `cond` / evaluator: завершено через M12-M18 (детально нижче), розбито на під-кроки.
...
- ✅ **M32 — `equal?` з `core.my`, підтверджено реальним прогоном з першого разу (2026-08-12)**: `(def equal? (lambda (a b) (cond ((atom a) (cond ((atom b) (eq a b)) (t '()))) ((atom b) '()) (t (cond ((equal? (car a) (car b)) (equal? (cdr a) (cdr b))) (t '()))))))` — самореференційний (на відміну від M30/M31's взаємної рекурсії), тож один letrec-плейсхолдер, той самий механізм, що й M28. Перший bootstrap-приклад із трьома top-level clause в `cond` (не два) — підтверджує, що загальний рекурсивний cond-диспетчер (M14) справді обробляє N clause, не лише два. Тестові дані: `(p . q)` побудовано ДВІЧІ як дві окремі cons-комірки з однаковим вмістом (не одна й та сама адреса), доводячи структурне, а не pointer-порівняння — саме те, чим `equal?` відрізняється від `eq`. `(equal? '(p . q) '(p . q)) => t`: `R9 = TAG:2 VAL:934` (не NIL; апаратний `eq` повертає сам символ-операнд як truthy-значення при збігу, не окремий TRUE-маркер — той самий патерн, що й M05's `tb_atom_eq.sv`). Edge-case checklist перед написанням коду зібрано з двох незалежних джерел через my-lisp TCP oracle (OpenCode й my-lisp агенти, mailbox id 15/16/19) — цикли підтверджено поза межами: мова не має `set-cdr!`/мутації, тож самореференційний cons неможливо збудувати з коду користувача, `equal?` ніколи не побачить циклічну структуру. [tb_bootstrap_equal.sv](../fpga/sim/tb_bootstrap_equal.sv), [bootstrap_equal_demo.asm](../bootstrap_equal_demo.asm), [equal-oracle-checklist.my](equal-oracle-checklist.my).

- ✅ **M33 — ISA 1.1 extended boot register inputs (2026-08-24)**: legacy `u16-le word-count + words` images remain byte-identical. An optional header with bit 15 set carries up to 16 `(register, tagged-word)` initializers before the unchanged program image. The register file is reset by board reset and gives boot writes priority until `boot_done`; CPU execution cannot race initialization. [tb_bootloader_register_init.sv](../fpga/sim/tb_bootloader_register_init.sv) sends R0=FIXNUM(3), R1=FIXNUM(4), then `ADD R2,R0,R1; HALT` and observes R2=FIXNUM(7). This is the RTL foundation for host-staged CML→FPGA payload edges; physical-board and CML integration evidence remain separate next steps.
