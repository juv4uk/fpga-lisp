...
# Звіт про тестування fpga-lisp (Ukrainian)

**Дата:** 2026-08-17 (оновлення RTL-SIM-PASS: 2026-08-18)  
**HEAD:** `5bcc99c` (RTL-SIM-PASS на коміті `9b529f2`)  

Цей звіт підтверджує виправлення кількох критичних помилок: 
подвійного збільшення лічильника команд (PC++) під час операції `FETCH_PAIR` 
(`CAR` + `CDR` за один прохід) та помилки на одиницю (off-by-one) при 
перевірці розміру купи (heap).

Спочатку тестування проводилося на Python-моделі (MODEL-PASS). Пізніше 
(2026-08-18) було додано підтвердження через реальну симуляцію `iverilog` 
(RTL-SIM-PASS).

## Знахідки від 2026-08-18 (щодо RTL-SIM-PASS)
Під час переходу від Python-моделі до реального `iverilog`, початковий 
тестбенч (`tb_fetch_pair.sv` на `8c4a12b`) взагалі не скомпілювався. 
Було виявлено дві помилки у **самому тестбенчі** (а не в RTL, який він тестував):
1. **Спроба обійти FSM через white-box тестування не спрацювала.** 
   Симулятор не дозволяє напряму привласнювати значення внутрішнім 
   безперервним (continuous) сигналам на зразок `boot_done`. Рішення: 
   завантажувати тестову програму через реальний UART-завантажувач.
2. **Кодування інструкцій було зворотним.** Функції кодування інструкцій 
   у тестбенчі збирали слова неправильно порівняно зі справжнім 
   `instruction_decoder.sv`. Значення `OP_*` також не збігалися.

Після виправлення тестбенча реальний RTL пройшов тест успішно з першої 
спроби. Це підтверджує, що сам RTL для `FETCH_PAIR` від початку був 
правильним. Регресійний набір з 33 тестбенчів також успішно пройшов.

## Проведені тести
1. **Test 1 (FETCH_PAIR happy path):** Підтверджує, що `CAR` і `CDR` 
   завантажуються в правильні регістри без пропуску наступної інструкції 
   (відсутній подвійний PC++).
2. **Test 2 (Heap capacity):** Доводить, що доступно рівно 4096 комірок 
   (від 0 до 4095). Аллокація 4097-ї комірки повертає помилку `HEAP_FULL`.
3. **Test 3 (FETCH_PAIR on non-cons):** Доводить, що спроба викликати 
   `FETCH_PAIR` на значенні, що не є вказівником (не `TAG_CONS`), 
   генерує апаратну помилку типу.
4. **Лічильники продуктивності:** Цикли, операції з купою та кількість 
   звернень відповідають очікуванням.

## Що НЕ тестувалося
- **Унікальність цільових регістрів:** Випадок, коли `rd == rs2` 
  (`CAR` переписується `CDR`), має відхилятися асемблером, але апаратно 
  це поки не перевіряється.
- **Синтез і фізичне апаратне забезпечення** розглядалися окремо в інших 
  звітах і документах.
