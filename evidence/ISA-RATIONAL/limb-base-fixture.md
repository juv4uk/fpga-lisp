...
- **G1 Канонічність на FPGA-бік:** без хвостових нуль-лимбів base-2^24,
  єдине кодування нуля — інакше eq розходиться. **ЗАКРИТО 2026-09-12**
  (`evidence/ISA-RATIONAL/g1-canonical-zero-check.my`, my-lisp-скрипт,
  прогнано проти реального `my-lisp.exe`): пінує раніше незафіксовану
  деталь дизайну — нуль кодується ПОРОЖНІМ limb-ланцюжком (аналог
  Rust's empty `Vec<u32>`), не одним лимбом зі значенням 0; довільне
  ненульове значення завжди має ненульовий старший (most-significant)
  лимб за побудовою алгоритму кодування (доведено конструктивно, не
  лише перевірено емпірично). Round-trip encode/decode перевірено на
  264 значеннях: явний boundary-набір (0, 1, 2^24±1, 2^32±1, 2^48) +
  усі операнди вже наявного G2-корпусу (`g2_differential_corpus.json`,
  sakshi). Fail-closed шлях підтверджено (навмисно зіпсована перевірка
  реально падає з exit code 1).
...
- **G4 28-bit payload переповнення** → named failure АБО перехід у
  ...
  **ХАРАКТЕРИЗОВАНО 2026-09-12** (`fpga/sim/tb_overflow_add.sv`,
  `overflow_add_demo.asm`, реальний iverilog, в CI): підтверджено
  емпірично, що чинний hardware `ADD` (звичайний `TAG_FIXNUM`-примітив,
  не пов'язаний з ще не збудованим `TAG_BIGNUM`) МОВЧКИ переповнюється
  за модулем 2^28 -- `2^27 + 2^27` (=2^28, на одиницю більше за
  максимальне представиме значення) дає `0`, без прапорця помилки, без
  розрізнюваного сигналу збою. Тобто: якщо/коли `TAG_BIGNUM`
  зʼявиться, детекція переповнення НЕ МОЖЕ покладатись на саму
  інструкцію `ADD` -- вона фізично не має виходу для цього. Запропонований
  (не ратифікований) механізм: bounds-перевірка операндів ДО виклику
  апаратного `ADD` (host-side/microcode), а не після. Це характеризація
  й пропозиція, не ратифіковане архітектурне рішення -- потребує того ж
  крос-репо review, що й будь-яка інша зміна ISA-поверхні.
