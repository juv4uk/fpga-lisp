...
HARDWARE_EQUIVALENT = [
    ("(length (quote (radio antenna signal)))", "M28/M29", "same op, demo uses '(a b c)"),
    ("(length (quote ()))", "M28/M29", "letrec base case (atom? -> 0), not separately demo'd with literal '() but same code path"),
    ("(reverse (quote (radio antenna signal)))", "M30", "same op, demo uses '(a b c)"),
    ("(append (quote (radio)) (quote (antenna signal)))", "M31", "same op, demo uses '(a b) '(c d)"),
    ("(equal? (quote (1 (2 3) 4)) (quote (1 (2 3) 4)))", "M32", "same op, demo uses '(p . q) '(p . q) (dotted pair, not nested list -- exercises the same recursive car/cdr walk either way)"),
    ("(equal? (quote (p . 0)) (cons (quote p) 0))", "M32", "dotted-pair form, closer to M32's actual literal data than the nested-list fixture above"),
    ("(atom (quote radio))", "M05", "ATOM primitive"),
    ("(eq (quote radio) (quote radio))", "M05", "EQ primitive"),
    ("(car (quote (radio antenna)))", "M03/M04", "CAR primitive"),
    ("(cdr (quote (radio antenna)))", "M04", "CDR primitive"),
    ("(cons (quote radio) (quote (antenna)))", "M03", "CONS primitive"),
    ("(cond (() (quote wrong)) (t (quote right)))", "G8/M07", "JF truthiness"),
    ("(quote radio)", "M13", "eval-quote"),
    ("(second (quote (radio antenna signal)))", "M20", "same op, demo uses '(x y z)"),
    ("(third (quote (radio antenna signal)))", "M25", "same op, demo uses '(w x y z)"),
    ("(not (quote ()))", "M21", "same op"),
    ("(not (quote radio))", "M21", "same op"),
